...
import {selectPhoneNumber, selectAuthorizationState, selectMonkey} from 'selectors';
import {createDiv, createText, destroyCallbacks} from 'ui';
import WaitPhone from './WaitPhone';
import WaitCode from './WaitCode';
import WaitPassword from './WaitPassword';
import WaitRegistration from './WaitRegistration';
...
const getMonkey = monkey => `<tgs-player
    class="${css.monkeyImage}"
    autoplay
    mode="normal"
    src="./monkey/${monkey}.tgs"
>
</tgs-player>`;
...
const setLogoImage = logoNode => (monkey) => {
    if (monkey) {
        logoNode.innerHTML = getMonkey(monkey);
    } else {
...
const setHeaderText = headerText => (authorizationState) => {
    if (authorizationState === 'authorizationStateWaitCode') {
        ...
        headerText.textContent = 'Your Name';
    } else if (authorizationState === 'authorizationStateWaitPassword') {
        headerText.textContent = 'Enter a Password';
    } else {
...
const setHelpText = helpText => (authorizationState) => {
    if (authorizationState === 'authorizationStateWaitCode') {
        ...
        helpText.textContent = 'Your account is protected with additional password.';
    } else {
...
const renderControl = (controlNode, authorizationState, control) => {
    ...
    if (authorizationState === 'authorizationStateWaitRegistration') {
        return WaitRegistration(controlNode);
    }

    if (authorizationState === 'authorizationStateWaitPassword') {
        return WaitPassword(controlNode);
    }
...
const LoginLayout = (parentNode) => {
    ...
    callbacks.push(subscribeSelector(selectMonkey, setLogoImage(logoNode)));
